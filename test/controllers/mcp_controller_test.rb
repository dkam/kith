...
class McpControllerTest < ActionDispatch::IntegrationTest
  ...
  test "a post that is only photographs still says something" do
    post = actors(:alice).posts.create!(title: "For now", audience: :followers, published_at: Time.current)
    embed_photo(post, filename: "landscape.jpg")
  ...
  # A draft has no published_at — that is what makes it a draft — so there is
  # no date to give. Its author may read it, here as on its own page.
  test "your own draft reads back as a draft rather than a date" do
    text = invoke("post", id: posts(:alice_draft).id)

    assert_includes text, "Not finished yet"
    assert_includes text, "draft"
  end

  test "somebody else's draft answers exactly as one that was never written" do
    hidden = invoke("post", id: posts(:carol_draft).id, expect_error: true)
    absent = invoke("post", id: 0, expect_error: true)
