...
class McpWritingTest < ActionDispatch::IntegrationTest
  ...
  # The tool answers "Posted.", so it had better be. A post with no
  # published_at is a draft — visible to its author and nobody else, and never
  # fanned out — which is exactly what an agent's post silently became when the
  # draft work met the MCP work.
  test "a post written this way is published, not left as a draft, and reaches a follower" do
    perform_enqueued_jobs { invoke("write_post", body: "Out in the world.") }

    post = Post.order(:id).last
    assert_predicate post, :published?
    assert FeedItem.exists?(member: members(:bob), post: post), "bob follows alice and never saw it"
  end
